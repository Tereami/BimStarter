...
namespace ViewsSheetsTools
{
    ...
    public class App : IExternalApplication
    {
        public Result OnStartup(UIControlledApplication application)
        {
            ...
            string projectName = nameof(ViewsSheetsTools);
            RibbonPanel panel = application.CreateRibbonPanel(tabName, $"{projectName} panel");
            _ = panel.AddItem(new PushButtonData(
                nameof(CommandOpenSheets),
                nameof(CommandOpenSheets),
                assemblyPath,
                $"{projectName}.{nameof(CommandOpenSheets)}")
                ) as PushButton;

            _ = panel.AddItem(new PushButtonData(
                nameof(CommandSetNumber),
                nameof(CommandSetNumber),
                assemblyPath,
                $"{projectName}.{nameof(CommandSetNumber)}")
                ) as PushButton;

            _ = panel.AddItem(new PushButtonData(
                nameof(CommandViewTemplate),
                nameof(CommandViewTemplate),
                assemblyPath,
                $"{projectName}.{nameof(CommandViewTemplate)}")
                ) as PushButton;
